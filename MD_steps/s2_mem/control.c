...
int main(int argc, char *argv[]) {
  ...
  for (i = 1; i < Ndim; i++) {
    int temp = i * Nbody;
    f[i][0] = f[0][0] + temp;
    pos[i][0] = pos[0][0] + temp;
    velo[i][0] = velo[0][0] + temp;
    delta_pos[i][0] = delta_pos[0][0] + temp * Nbody;
  }
  ...
  for (i = 0; i < Nbody; i++) {
    fscanf(in, "%16le%16le%16le%16le%16le%16le%16le%16le%16le\n", mass + i,
           radius + i, vis + i, &pos[Xcoord][i], &pos[Ycoord][i],
           &pos[Zcoord][i], &velo[Xcoord][i], &velo[Ycoord][i],
           &velo[Zcoord][i]);
  }
  fclose(in);
  ...
  for (j = 1; j <= Nsave; j++) {
    ...
    for (i = 0; i < Nbody; i++) {
      fprintf(out, "%16.8E%16.8E%16.8E%16.8E%16.8E%16.8E%16.8E%16.8E%16.8E\n",
              mass[i], radius[i], vis[i], pos[Xcoord][i], pos[Ycoord][i],
              pos[Zcoord][i], velo[Xcoord][i], velo[Ycoord][i],
              velo[Zcoord][i]);
    }
    fclose(out);
